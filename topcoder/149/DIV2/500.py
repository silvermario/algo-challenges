...
class BigBurger(object):
	def maxWait(self, arrival, service):
		...
		longest_wait_time = 0
		time = arrival[0] + service[0]										# time after first served client
		for arrival_time, service_time in zip(arrival[1:], service[1:]):	# check next clients
			if arrival_time >= time:										# if they came after the previous clients was already served continue to the next one
				time = arrival_time + service_time
			else:
				wait_time = time - arrival_time
				longest_wait_time = max(wait_time, longest_wait_time)		# has the client waited more than others?
				time += service_time
...
assert BigBurger().maxWait((3,3,9),(2,15,14)) == 11
assert BigBurger().maxWait((2,10,11),(3,4,3)) == 3
assert BigBurger().maxWait((2,10,12),(15,1,15)) == 7
assert BigBurger().maxWait((182,),(11,)) == 0
